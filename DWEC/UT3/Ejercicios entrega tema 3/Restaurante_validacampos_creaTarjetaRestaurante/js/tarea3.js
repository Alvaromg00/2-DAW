...
boton.addEventListener('click', (event)=>{
    ...
    if(nombreInvalido.style.display=="block" || descripcionInvalido.style.display=="block" || cocinaInvalido.style.display=="block" || telefonoInvalido.style.display=="block" || fotoInvalido.style.display=="block" || oculto==false){
...
form.addEventListener('submit', (event) => {
    ...
    function isAbierto(diasApertura) {
        const diaActual = new Date().getDay(); // 0 para Domingo, 1 para Lunes, etc.
    
        const diasSeleccionados = Array.from(diasApertura).filter((checkbox) => checkbox.checked);
        
        // Se mostrará "Abierto" si el día actual está seleccionado y hay otros días seleccionados
        return diasSeleccionados.length >= 1 && diasSeleccionados.some((checkbox) => parseInt(checkbox.value, 10) === diaActual);
    }
